...
export function TasksView({ tasks, refetch }: { tasks: Task[]; refetch: () => void }) {
  ...
  const NavItem = ({ id, icon: Icon, label, count, colorClass = "text-[#202020]", activeBg = "bg-[#ffefe5]", iconColor = "text-[var(--color-ink-3)]", onClick }: any) => {
    ...
    return (
      ...
      >
        ...
        {count > 0 && (
          <span className={`text-xs ${isActive ? 'text-[#db4c3f] font-semibold' : 'text-[var(--color-ink-3)] font-normal'}`}>{count}</span>
        )}
      </button>
    );
  };
    ...
    <div className="flex h-full absolute inset-0 bg-white font-sans text-[#202020]" style={{ fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif' }}>
      ...
      <div className="w-[300px] bg-[#fafafa] flex flex-col pt-5 pb-4 transition-all duration-300">
        ...
        <div className="px-2 mb-2">
          <button className="flex items-center gap-3 text-[var(--color-ink-3)] hover:text-[var(--color-ink)] hover:bg-gray-100 w-full px-3 py-1.5 rounded-lg transition-colors cursor-text text-[13px] font-medium">
            <Search size={18} />
            Buscar
          </button>
        </div>
        ...
        <div className="px-6 pt-5 pb-1 text-[11px] font-bold text-[var(--color-ink-3)] flex justify-between items-center group cursor-pointer hover:bg-gray-100 rounded-lg mx-2 transition-colors">
          Favoritos
        ...
        <div className="mt-auto px-4 text-xs text-[var(--color-ink-3)] flex items-center gap-1 font-medium hover:text-[var(--color-ink-2)] cursor-pointer">
        ...
        <div className="max-w-4xl mx-auto border-b border-transparent">
          ...
          {activeSection === 'breve' && (
            <div className="mb-6 flex items-baseline gap-2">
              ...
              <span className="text-sm font-normal text-[var(--color-ink-3)] cursor-pointer hover:underline">Março {new Date().getFullYear()} <ChevronDown size={14} className="inline opacity-80" /></span>

              <div className="ml-auto border border-gray-200 rounded-md px-2 py-1 text-xs font-medium text-[var(--color-ink-2)] flex items-center gap-1 hover:bg-gray-50 cursor-pointer">
          ...
          {activeSection === 'hoje' && (
            <div className="mb-6">
              ...
              <div className="text-xs text-[var(--color-ink-3)] flex items-center gap-2 mt-1">
              ...
              <div className="flex gap-6 w-full items-start overflow-x-auto pb-4">
                  ...
                  <div className="min-w-[300px] flex-1">
                    <div className="flex items-center justify-between border-b border-gray-100 pb-2 mb-2">
                      <h3 className="font-bold text-sm text-[#202020] flex items-center gap-2">
                        Atrasada <span className="text-[11px] text-[var(--color-ink-3)] font-normal">{tasksAtrasadas.length}</span>
                      </h3>
                ...
                <div className="min-w-[300px] flex-1">
                  <div className="border-b border-gray-100 pb-2 mb-2 text-sm font-bold flex items-center gap-2 text-[#202020]">
                    <span className="capitalize">{new Date(strToday + 'T12:00:00').toLocaleDateString('pt-BR', { month: 'short', day: 'numeric' }).replace('.', '')}</span> · Hoje <span className="text-[11px] font-normal text-[var(--color-ink-3)]">{tasksHoje.length}</span>
                  </div>
                  <div className="flex flex-col gap-1">
                    {tasksHoje.map(t => <TodoistTaskRow key={t.id} task={t} onComplete={handleComplete} />)}
                  </div>
                  <button onClick={() => { setActiveSection('hoje'); setAddingTask('hoje_inline'); }} className="flex items-center gap-2 text-[var(--color-ink-3)] hover:text-[#db4c3f] mt-1 group text-[13px] cursor-pointer w-full p-2 py-1.5 rounded-lg hover:bg-gray-50 transition-colors">
                ...
                <div className="min-w-[300px] flex-1">
                  <div className="border-b border-gray-100 pb-2 mb-2 text-sm font-bold flex items-center gap-2 text-[#202020]">
                    <span className="capitalize">{new Date(strTmrw + 'T12:00:00').toLocaleDateString('pt-BR', { month: 'short', day: 'numeric' }).replace('.', '')}</span> · Amanhã <span className="text-[11px] font-normal text-[var(--color-ink-3)]">{tasksBreve.filter(t => t.due_date === strTmrw).length}</span>
                  </div>
                  <div className="flex flex-col gap-1">
                  ...
                  {addingTask === strTmrw ? (
                    ...
                    <button onClick={() => setAddingTask(strTmrw)} className="flex items-center gap-2 text-[var(--color-ink-3)] hover:text-[#db4c3f] mt-1 group text-[13px] cursor-pointer w-full p-2 py-1.5 rounded-lg hover:bg-gray-50 transition-colors">
                      <Plus size={16} className="text-[#db4c3f] opacity-80" /> Adicionar tarefa
                    </button>
                  )}
                </div>
            ...
            {activeSection === 'hoje' && (
              <div className="flex flex-col">
                  ...
                  <div className="mb-6">
                    <div className="flex items-center justify-between border-b border-gray-100 pb-2 mb-2">
                      <h3 className="font-bold text-sm text-[#202020] flex items-center gap-1">
                        Atrasada
                        <ChevronDown size={14} className="text-[var(--color-ink-3)] opacity-80" />
                      </h3>
                ...
                {addingTask === 'hoje_inline' ? (
                  ...
                  <button onClick={() => setAddingTask('hoje_inline')} className="flex items-center gap-3 text-[var(--color-ink-3)] hover:text-[#db4c3f] group w-full text-left py-2 cursor-pointer transition-colors mt-2 text-[14px]">
            ...
            {(activeSection === 'entrada' || activeSection === 'calendario') && (
              <div className="flex flex-col">
                ...
                {addingTask === 'generic' ? (
                  ...
                  <button onClick={() => setAddingTask('generic')} className="flex items-center gap-3 text-[var(--color-ink-3)] hover:text-[#db4c3f] group w-full text-left py-2 cursor-pointer transition-colors mt-2 text-[14px]">
                    <Plus size={20} className="text-[#db4c3f] rounded-full group-hover:bg-[#ffefe5] p-0.5 transition-colors" />
                    <span>Adicionar tarefa</span>
                  </button>
                )}
    ...
    <div className={`group flex items-start flex-col sm:flex-row shadow-[0_1px_3px_rgba(0,0,0,0.02)] sm:items-center gap-2.5 py-2.5 border border-gray-200 mt-2 hover:bg-white rounded-xl transition-all cursor-pointer bg-white overflow-hidden px-3`}>
      <div className="flex w-full items-start gap-2.5">
        ...
        <div className="flex-1 flex flex-col pt-[1px] min-w-0">
          ...
          {task.description && (
            <p className="text-[12px] text-[var(--color-ink-3)] truncate mt-0.5">
              {isLink ? (
                <a href={task.description} target="_blank" rel="noreferrer" className="text-[var(--color-ink-3)] underline decoration-gray-300 hover:text-blue-500">{task.description}</a>
              ) : task.description}
            </p>
          )}
          <div className="flex items-center gap-2 mt-1.5 flex-wrap">
            {task.due_date && (
              <div className={`flex items-center gap-1 text-[11px] ${isOverdue ? 'text-[#db4c3f] font-medium' : 'text-[var(--color-ink-3)]'}`}>
            ...
            {task.project && task.project !== 'Entrada' && (
              <div className="text-[11px] text-[var(--color-ink-3)] flex items-center gap-1 md:ml-auto">
  ...
  return (
    <div className={`border border-gray-200 rounded-xl p-3 bg-white shadow-sm mt-2 transition-all w-full focus-within:border-gray-300`}>
      <input
        ...
        className="w-full text-sm font-semibold text-[#202020] focus:outline-none placeholder:text-[var(--color-ink-3)] mb-1"
      />
      <textarea
        ...
        rows={1}
        className="w-full text-xs text-[var(--color-ink-2)] focus:outline-none placeholder:text-[var(--color-ink-3)] resize-none mt-1 h-6"
      />
      <div className="flex justify-end gap-2 mt-2 pt-2 border-t border-gray-100">
        <button onClick={onCancel} className="px-3 py-1.5 text-xs font-semibold text-[var(--color-ink-2)] bg-gray-50 hover:bg-gray-100 rounded-lg transition-colors cursor-pointer border border-gray-200">
          Cancelar
        </button>
        <button
          onClick={onSave}
